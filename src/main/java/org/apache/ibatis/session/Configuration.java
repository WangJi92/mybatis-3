...
public class Configuration {
   ...
   * 即从经典数据库列名 A_COLUMN 到经典 Java 属性名 aColumn 的类似映射。	true | false	False
   */
   ...
   * 具体可参考相关驱动文档或通过测试这两种不同的模式来观察所用驱动的结果。	true | false	true
   */
   ...
   * 为了使用该特性，你的工程必须采用Java 8编译，并且加上-parameters选项。（从3.4.1开始）	true | false	true
   */
   ...
   * 比如 NULL、VARCHAR 或 OTHER。	JdbcType 常量. 大多都为: NULL, VARCHAR and OTHER	OTHER
   */
  ...
  /**
   * 为驱动的结果集获取数量（fetchSize）设置一个提示值。此参数只可以在查询设置中被覆盖。	任意正整数	Not Set (null)
   */
   ...
   * 这个类必须包含一个签名方法static Configuration getConfiguration(). (从 3.2.3 版本开始)	类型别名或者全类名.	Not set
  ...
  /**
   * Mybatis使用package+Mapper+method全限名作为key，去xml内寻找唯一sql来执行的。 mappedStatement对象对应Mapper配置文件中的一个select/update/insert/delete节点，主要描述的是一条SQL语句
   */
  ...
  /**
   * 保存已经加载过、解析过的Mapper数据
   * {@linkplain MapperAnnotationBuilder#parse() 看看当前Mapper是否已经被加载过啦}
   *
   * xml 唯一标识为 namespace +class全限定名称
   * mapper class 接口  唯一标识为 class.toString
   */
  ...
  /**
   *
   * @param executor  当前的执行器
   * @param mappedStatement  具体的某条SQL  语句保护的信息
   * @param parameterObject  传递进来的参数
   * @param rowBounds  分页参数处理
   * @param resultHandler
